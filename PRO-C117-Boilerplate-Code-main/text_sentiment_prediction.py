...
def predict(text):
    ...
    if text != '':
        ...
        predicted_class_label = np.argmax(model.predict(training_padded))
        for key, value in emo_code_url.items():
            if value[0]==predicted_class_label: 
                predicted_emotional_img_url = value[1] #passing image in array to predicted_emotional_img_url
                predicted_emotion = key #the key in the key:value format
        return predicted_emotion, predicted_emotional_img_url
